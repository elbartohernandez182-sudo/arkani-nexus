...
def parsear_accion(texto: str) -> dict:
    ...
    # Extraer parametros - FIX DEFINITIVO
    ...
    if match:
        ...
        try:
            ...
            # Intentar comillas dobles: clave="valor"
            params = re.findall(r'(\w+)="([^"]+)"', params_str)
            for k, v in params:
                resultado["parametros"][k] = v
            # Si no capturo nada, intentar comillas simples: clave='valor'
...
def correr_agente(objetivo: str, verbose: bool = True) -> str:
    ...
    for paso in range(MAX_PASOS):
        ...
        if verbose:
            if parsed:
                print(f"Pensamiento: {parsed.get('pensamiento','')[:100]}")
    ...
    return f"Agente completo {MAX_PASOS} pasos. Revisa el log para detalles."
